...
const FILTERS: { value: Filter; label: string; href: string }[] = [
  { value: "all", label: "All", href: "/notebook" },
  { value: "improve", label: "Could be better", href: "/notebook?verdict=improve" },
  { value: "error", label: "Mistakes", href: "/notebook?verdict=error" },
];
...
function parseFilter(value: string | string[] | undefined): Filter {
  return value === "error" || value === "improve" ? value : "all";
}
...
export default async function NotebookPage({ searchParams }: PageProps<"/notebook">) {
  const filter = parseFilter((await searchParams).verdict);
  const all = buildNotebook(allSessions());
  const entries = filter === "all" ? all : all.filter((e) => e.verdict === filter);
      ...
      <nav aria-label="Filter by verdict" className="mt-6 flex flex-wrap gap-x-4 gap-y-1 text-sm">
        {FILTERS.map((f) => (
          <Link
            ...
            className="text-zinc-500 underline-offset-4 hover:text-zinc-900 hover:underline aria-[current]:font-medium aria-[current]:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 dark:aria-[current]:text-zinc-100"
          >
            {f.label}
          </Link>
        ))}
        ...
        <div className="mt-10 flex flex-col gap-12">
          {groups.map((group) => (
            <section key={group.sessionId} aria-labelledby={`session-${group.sessionId}`}>
              ...
              >
                <Link href={`/s/${group.sessionId}`} className="underline-offset-4 hover:underline">
                  {group.topic}
                </Link>
